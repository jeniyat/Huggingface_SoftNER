...
@add_start_docstrings(
    """JT: added this class for BertForTokenClassification for the soft-ner task """,
    BERT_START_DOCSTRING,
)
class BertForTokenClassification_Soft_NER(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
@add_start_docstrings(
    """JT: added this class for BertForTokenClassification for the segmenter task """,
    BERT_START_DOCSTRING,
)
class BertForTokenClassification_Seg(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        ...
        if self.attn_flag:
            ...
            if self.org_flag:
                ...
                self.classifier = nn.Linear(config.hidden_size + 850*2, config.num_labels)
        self.classifier = nn.Linear(3174, config.num_labels)

        vocab_size = 16255
        freq_embed_dim=102
        self.freq_embed = nn.Embedding(vocab_size, freq_embed_dim)
        word_freq_embed = np.load('freq_embeds.npy')
        self.freq_embed.weight = nn.Parameter(torch.FloatTensor(word_freq_embed))


        self.ctc_embed = nn.Embedding(60, 768)
        self.md_embed = nn.Embedding(60, 768)
        self.init_weights()
    ...
    Examples::
        ...
        # print("labels_ctc beofre: ",labels_ctc)
        sequence_output = outputs[0]
        labels_ctc[labels_ctc<0]=0
        labels_md[labels_md<0]=0
        # print("labels_ctc aftre: ",labels_ctc)
        # print("*********************")
        # print(labels_seg)
        #print(labels)
        #import sys
        #sys.exit()
        ctc_embeds = self.ctc_embed(labels_ctc)
        md_embeds = self.md_embed(labels_md)
        freq_embeds = self.freq_embed(freq_ids)
        # print("sequence_output: ",sequence_output)
        # print(ctc_embeds)
        # print("seg_embeds: ",seg_embeds)
        #import sys
        #sys.exit()
        #sequence_output = torch.cat([sequence_output, ctc_embeds, seg_embeds], 2)
        sequence_output = torch.cat([sequence_output, ctc_embeds, md_embeds], 2)

        # sequence_output = sequence_output
        # print("sequence_output.size(): ",sequence_output.size())
        # print("seg_embeds.size(): ",seg_embeds.size())
        sequence_output = torch.cat((sequence_output, md_embeds, freq_embeds), 2)
...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        # print("sequence_output.size(): ",sequence_output.size())
        # print("seg_embeds.size(): ",seg_embeds.size())
        sequence_output = torch.cat((sequence_output, seg_embeds, freq_embeds), 2)
